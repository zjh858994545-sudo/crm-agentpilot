...
- Publish Agent run, tool-call, and confirmed CRM task events through an outbox-backed event layer. Confirmed CRM task events are written to `agent_outbox_event` inside the confirmation transaction and dispatched after commit. Agent run and tool-call events are audit events that use the same table for at-least-once retry, but they are not transactionally bound to a long-running `chat()` transaction because the Agent path intentionally avoids holding a database transaction across external LLM HTTP calls. The default is log-only for stable local demos; setting `AGENT_EVENTS_KAFKA_ENABLED=true` sends the same events to Kafka topics.
